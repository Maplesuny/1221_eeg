...
export default {
    setup () {
        ...
        let aaaa = ref([])
        let bbbb = ref([])
        aaaa.value = [[1065, 1204], [298, 399]]  // range裡面的樣子
        bbbb.value = [[[1065, 1204], [298, 399]], [[20, 1204], [298, 399]]]

        console.log('aaaaa', aaaa.value)
        console.log('bbbb', bbbb.value)

        let get_last_brush = ref([]
        )
        ...
        get_last_brush.value.push(brush_ares0.value)

        // get_last_brush.value.shift()
        watch(get_last_brush.value, () => {
            console.log('get_last_brush', get_last_brush.value)
            console.log('get_last_length', get_last_brush.value[0].length)
        ...
        const { axioss, channel_name_function, convert_sec, convert_sec2, roundToTwo } = option("Echarts")
                        ...
                        onclick: function () {
                            ...
                            myChart.dispatchAction({
                                ...
                                areas: [{
                                    xAxisIndex: 0,
                                    brushType: 'rect',
                                    range: [[1388, 1538], [186, 298]]
                                },
                                {
                                    xAxisIndex: 1,
                                    brushType: 'rect',
                                    range: [[1065, 1204], [298, 399]]
                                }
                                ]
                            });
        ...
        async function dynamic_10sdata () {
            ...
            console.log('dynamic 秒數: ', dynamic_start, dynamic_end + ' 頁數: ', page.value)
            let url =
                "http://127.0.0.1:8000/api/v1/eegData?start_time=" +
                dynamic_start +
                "&end_time=" +
        ...
        async function SetOption (url, start, end) {
            ...
            myChart.on('brushSelected', function (params) {
                ...
                if (brushComponent.areas[0] !== undefined) {
                    ...
                    select(params, type);
                    console.log('burshselected_', brush_ares0.value)
                }
            })
        ...
        function select (params, brushType) {
            ...
            // 儲存所框選的座標
            brush_ares0.value.push([params.batch[0].areas[0].range[0], params.batch[0].areas[0].range[1]])
            // 判斷X軸是LineX 還是 rect
            if (brushType !== "lineX") {
        ...
        return {
            page, Minpage, Maxpage, jump_to,
            pre_button, next_button, send_page
        }
